...
// @namespace    -
// @version      -
// @description  un script q te hace cambiar el status dx
// @author       Igna
// @match        *://open.spotify.com/*
// @icon         https://cdn.discordapp.com/attachments/1203575626825728101/1233141538129973360/nQQGn28.jpg?ex=662d558d&is=662c040d&hm=96841b395a0e6d493411a241bda5c12b0b2c35a82bd877bdce8230feedb57d21&
// @grant        none
...
<div id="menu-UI" class="hid-anim">
    ...
    <div id="menu-contents">
        <div id="run-tab" class="tab-content act">
            ...
            <div id="ss-buttons">
                <button id="start" class="button1">Iniciar</button>
                <button id="stop" class="button1">Detener</button>
            </div>
        </div>
        <div id="settings-tab" class="tab-content hid">
            <div class="settings">
                ...
                <div class="option">
                    <label for="user-token">Tu Token :v :</label>
                    <input type="text" id="user-token" class="text-input1">
                    <button id="check-token" class="button1">check token osea pibe</button>
                </div>
                <div class="option">
            ...
            <div class="settings">
                ...
                <div class="option">
                    <label for="enable-timestamp">Activar marca de tiempo dx</label>
                    <input type="checkbox" id="enable-timestamp" checked>
                </div>
                <div class="option">
                    <label for="enable-label">activar label ni idea</label>
                ...
                <div class="option">
                    <label for="enable-advanced-swt">opciones no se avanzadas pibe</label>
            ...
            <div class="settings">
                ...
                <div class="option">
                    ...
                    <select id="autooffset">
                        <option value="off" selected>apagao</option>
                        <option value="mode1">Ultimo nose request</option>
                        <option value="mode2">Promedio de 2 solicitudes</option>
                        <option value="mode3">Promedio de 10 solicitudes</option>
                        <option value="mode4">Promedio de 30 solicitudes</option>
                    </select>
            ...
            <div id="version">Version ni yo se pibe</div>
        </div>
...
let settings = {
    ...
    view: {
        ...
        advanced: {
            enabled: false,
            customEmoji: "💫",
            customStatus: "[{timestamp}] mira estas lyrics pibe {lyrics}"
        }
    },
    timings: {
        sendTimeOffset: 500,
...
$(document).keyup((e) => e.key === "F2" ? menu.toggleClass("act-anim").toggleClass("hid-anim") : false);
...
checkTokenButton.click(() => {
    ...
    if(!valid) return modal("Token checker amor", "tu token esta re muerto gato", { descriptionTextColor: "rgba(200, 0, 0, var(--alpha))" });
    modal("Token checker amor", "el token es valido VAMOOOO nuevo token (jodita)", { descriptionTextColor: "rgba(0, 200, 0, var(--alpha))" });
});
...
enableTimestampCheckbox.click(() => {
    ...
    statusPreview.text(getStatusString("sexosexosexo", 137000));
});
...
customEmojiHelp.click(() => {
    modal("Help", `
    <strong>Custom emoji</strong>Esta opción te permite agregar un emoji antes de tu estado.<br>
    Utilice un emoji Unicode. Puedes conseguirlo <a style="color: rgba(154, 154, 154, var(--alpha));" href="https://www.piliapp.com/emoji/list/">here</a>.
    `);
});
...
function changeStatusRequest(token, text, emoji) {
    ...
    $.ajax({
        url: "https://discordapp.com/api/v10/users/@me/settings",
        method: "PATCH",
        dataType: "json",
        headers: {
        ...
        statusCode: {
            ...
            401: () => {
                modal("Run", "token ded osea muerto dx", { descriptionTextColor: "rgba(200, 0, 0, var(--alpha))" });
                stopLog = true;
                stopped = true;
            }
        }
...
function loadSettings() {
    ...
    try {
        ...
        addLog(`nose hay un error parece :V <br><span style="color: rgba(150, 0, 0, var(--alpha));user-select: text;">${e}</span>`, "error");
    }
}
...
(async function playbackStateUpdater() {
    ...
    updatePlaybackState().always(async () => {
        if(errorCount >= 10) {
            addLog(" che hay un error y se detuvo el lyrics sex", "warning");
            stopLog = true;
            stopped = true;
...
(async function statusChanger() {
    setInterval(() => {
        if(startLog) {
            ...
            debugRequest10.text("Fetching...");
            debugRequest30.text("Fetching...");
            addLog("sexo lyrics esta prendido gg");
        }
        if(stopLog) {
            stopLog = false;
            debugLyrics.text("sexo lyrics esta detenido");
            debugRequest.text("sexo lyrics esta detenido");
            debugRequest2.text("sexo lyrics esta detenido");
            debugRequest10.text("sexo lyrics esta detenido");
            debugRequest30.text("sexo lyrics esta detenido");
            addLog("sexo lyrics esta detenido");
        }
        if(stopped) {
